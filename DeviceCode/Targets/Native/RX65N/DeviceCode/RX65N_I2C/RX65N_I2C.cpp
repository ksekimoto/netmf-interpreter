...
#define RX65N_SCL RX65N_GPIO::P12;
#define RX65N_SDA RX65N_GPIO::P13;

RX65N_I2C_Driver g_RX65N_I2C_Driver;
...
BOOL RX65N_I2C_Driver::Initialize(UINT32 ch)
{
    riic_t *riicp = RIIC[ch];

//    if (g_RX65N_I2C_Driver.m_initialized == FALSE) {
        g_RX65N_I2C_Driver.m_currentXAction = NULL;
        g_RX65N_I2C_Driver.m_currentXActionUnit = NULL;

        // CPU_GPIO_DisablePin((GPIO_PIN)RX65N_SCL, RESISTOR_DISABLED, 0, GPIO_ALT_MODE_1);
        // CPU_GPIO_DisablePin((GPIO_PIN)RX65N_SDA, RESISTOR_DISABLED, 0, GPIO_ALT_MODE_1);
        ...
        clear_i2c_line(ch);
        g_RX65N_I2C_Driver.m_initialized  = TRUE;
...
BOOL RX65N_I2C_Driver::Uninitialize(UINT32 ch)
{
    NATIVE_PROFILE_HAL_PROCESSOR_I2C();
    riic_t *riicp = RIIC[ch];

//    if (g_RX65N_I2C_Driver.m_initialized == TRUE) {
        g_RX65N_I2C_Driver.m_initialized = FALSE;
        // CPU_GPIO_EnableInputPin((GPIO_PIN)RX65N_SCL, FALSE, NULL, GPIO_INT_NONE, RESISTOR_DISABLED);
        // CPU_GPIO_EnableInputPin((GPIO_PIN)RX65N_SDA, NULL, GPIO_INT_NONE, RESISTOR_DISABLED);
...
void RX65N_I2C_Driver::ReadLastByte(UINT32 ch)
{
    riic_t *riicp = RIIC[ch];
    I2C_HAL_XACTION *xAction = g_RX65N_I2C_Driver.m_currentXAction;
    I2C_HAL_XACTION_UNIT *unit = g_RX65N_I2C_Driver.m_currentXActionUnit;
...
void RX65N_I2C_Driver::StopCondition(UINT32 ch)
{
    riic_t *riicp = RIIC[ch];
    riicp->ICSR2.BIT.NACKF = 0;
    riicp->ICSR2.BIT.STOP = 0;
...
void RX65N_I2C_Driver::Abort(UINT32 ch)
{
    UINT8 dummy;
    riic_t *riicp = RIIC[ch];
    I2C_HAL_XACTION *xAction = g_RX65N_I2C_Driver.m_currentXAction;
...
void RX65N_I2C_Driver::MasterXAction_Start(UINT32 ch, I2C_HAL_XACTION* xAction, bool repeatedStart)
{
    NATIVE_PROFILE_HAL_PROCESSOR_I2C();
    UINT8 address;
    UINT32 timeout;
...
//    if (g_RX65N_I2C_Driver.m_initialized == FALSE)
//        return;
    g_RX65N_I2C_Driver.m_currentXAction = xAction;
    g_RX65N_I2C_Driver.m_currentXActionUnit = xAction->m_xActionUnits[xAction->m_current++];
    I2C_HAL_XACTION_UNIT *unit = g_RX65N_I2C_Driver.m_currentXActionUnit;
    ...
    address |= unit->IsReadXActionUnit() ? RX65N_I2C_Driver::c_DirectionRead : RX65N_I2C_Driver::c_DirectionWrite;
    ...
    if (!unit->IsReadXActionUnit()) {
        while (riicp->ICSR2.BIT.TDRE == 0) ;
        i2c_IEN(ch, 1);
        riicp->ICIER.BYTE |= (RX65N_I2C::ICIER_TIE || RX65N_I2C::ICIER_TEIE);
    } else {
        ...
        while (riicp->ICSR2.BIT.RDRF == 0) ;
        i2c_IEN(ch, 1);
        riicp->ICIER.BYTE |= (RX65N_I2C::ICIER_RIE || RX65N_I2C::ICIER_NAKIE || RX65N_I2C::ICIER_SPIE);
...
void RX65N_I2C_Driver::MasterXAction_Stop(UINT32 ch)
{
    NATIVE_PROFILE_HAL_PROCESSOR_I2C();
    riic_t *riicp = RIIC[ch];
...
void RX65N_I2C_Driver::GetClockRate(UINT32 rateKhz, UINT8& clockRate, UINT8& clockRate2)
{
    UINT32 bgr;
    UINT8 div_idx = 3;
    if (rateKhz > 400)
...
void RX65N_I2C_Driver::WriteToSubordinate(UINT32 ch, I2C_HAL_XACTION_UNIT *unit)
{
    UINT8 *queueData;
    riic_t *riicp = RIIC[ch];
...
void RX65N_I2C_Driver::ReadFromSubordinate(UINT32 ch, I2C_HAL_XACTION_UNIT *unit)
{
    UINT8* queueData;
    riic_t *riicp = RIIC[ch];
...
void RX65N_I2C_ICEEI_ISR(UINT32 ch)
{
    riic_t *riicp = RIIC[ch];
#ifdef DEBUG_I2C_INT
...
void RX65N_I2C_ICRXI_ISR(UINT32 ch)
{
    riic_t *riicp = RIIC[ch];
    I2C_HAL_XACTION_UNIT *unit = g_RX65N_I2C_Driver.m_currentXActionUnit;
    I2C_HAL_XACTION *xAction = g_RX65N_I2C_Driver.m_currentXAction;
    ...
    if (unit->m_bytesToTransfer > 2) {
        if (unit->m_bytesToTransfer == 3)
            riicp->ICMR3.BIT.WAIT = 1;
        g_RX65N_I2C_Driver.ReadFromSubordinate(ch, unit);
    } else if (unit->m_bytesToTransfer == 2) {
        riicp->ICMR3.BIT.ACKBT = 1;
        g_RX65N_I2C_Driver.ReadFromSubordinate(ch, unit);
    } else {
        g_RX65N_I2C_Driver.ReadLastByte(ch);
    }
}

void RX65N_I2C_ICTXI_ISR(UINT32 ch)
{
    riic_t *riicp = RIIC[ch];
    I2C_HAL_XACTION_UNIT *unit = g_RX65N_I2C_Driver.m_currentXActionUnit;
    GLOBAL_LOCK(irq);
    ...
    if (unit->m_bytesToTransfer) {
        g_RX65N_I2C_Driver.WriteToSubordinate(ch, unit);
    } else {
        riicp->ICIER.BIT.TIE = 0;
    }
}

void RX65N_I2C_ICTEI_ISR(UINT32 ch)
{
    riic_t *riicp = RIIC[ch];
    I2C_HAL_XACTION *xAction = g_RX65N_I2C_Driver.m_currentXAction;
    I2C_HAL_XACTION_UNIT *unit = g_RX65N_I2C_Driver.m_currentXActionUnit;
    ...
    if (xAction->ProcessingLastUnit()) {
        g_RX65N_I2C_Driver.StopCondition(ch);
...
#endif
        g_RX65N_I2C_Driver.MasterXAction_Start(ch, xAction, true);
    }
}

void  RX65N_I2C_Driver::GetPins(UINT32 ch, GPIO_PIN& scl, GPIO_PIN& sda)
{
    NATIVE_PROFILE_HAL_PROCESSOR_I2C();
    scl = SCL_PIN[ch];
    sda = SDA_PIN[ch];
...
extern "C" {
#endif
void __attribute__ ((interrupt)) INT_Excep_RIIC0_EEI0(void) { RX65N_I2C_ICEEI_ISR(0); }
void __attribute__ ((interrupt)) INT_Excep_RIIC0_RXI0(void) { RX65N_I2C_ICRXI_ISR(0); }
void __attribute__ ((interrupt)) INT_Excep_RIIC0_TXI0(void) { RX65N_I2C_ICTXI_ISR(0); }
void __attribute__ ((interrupt)) INT_Excep_RIIC0_TEI0(void) { RX65N_I2C_ICTEI_ISR(0); }
void __attribute__ ((interrupt)) INT_Excep_RIIC1_EEI1(void) { RX65N_I2C_ICEEI_ISR(1); }
void __attribute__ ((interrupt)) INT_Excep_RIIC1_RXI1(void) { RX65N_I2C_ICRXI_ISR(1); }
void __attribute__ ((interrupt)) INT_Excep_RIIC1_TXI1(void) { RX65N_I2C_ICTXI_ISR(1); }
void __attribute__ ((interrupt)) INT_Excep_RIIC1_TEI1(void) { RX65N_I2C_ICTEI_ISR(1); }
void __attribute__ ((interrupt)) INT_Excep_RIIC2_EEI2(void) { RX65N_I2C_ICEEI_ISR(2); }
void __attribute__ ((interrupt)) INT_Excep_RIIC2_RXI2(void) { RX65N_I2C_ICRXI_ISR(2); }
void __attribute__ ((interrupt)) INT_Excep_RIIC2_TXI2(void) { RX65N_I2C_ICTXI_ISR(2); }
void __attribute__ ((interrupt)) INT_Excep_RIIC2_TEI2(void) { RX65N_I2C_ICTEI_ISR(2); }
void __attribute__ ((interrupt)) INT_Excep_RIIC3_EEI3(void) { RX65N_I2C_ICEEI_ISR(3); }
void __attribute__ ((interrupt)) INT_Excep_RIIC3_RXI3(void) { RX65N_I2C_ICRXI_ISR(3); }
void __attribute__ ((interrupt)) INT_Excep_RIIC3_TXI3(void) { RX65N_I2C_ICTXI_ISR(3); }
void __attribute__ ((interrupt)) INT_Excep_RIIC3_TEI3(void) { RX65N_I2C_ICTEI_ISR(3); }
#ifdef __cplusplus
}
#endif

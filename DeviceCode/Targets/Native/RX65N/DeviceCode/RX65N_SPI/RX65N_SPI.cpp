...
static unsigned char g_ier_bit[] = {
    7,      // IEN7
    2,      // IEN2
    5,      // IEN5
};
...
UINT8 SPI_PINS[] = {
    RX65N_GPIO::PC5,    // RSPCKA
    RX65N_GPIO::PC6,    // MOSIA
    RX65N_GPIO::PC7,    // MISOA
    RX65N_GPIO::PE5,    // RSPCKB
    RX65N_GPIO::PE6,    // MOSIB
    RX65N_GPIO::PE7,    // MISOB
    RX65N_GPIO::PD3,    // RSPCKC
    RX65N_GPIO::PD1,    // MOSIC
    RX65N_GPIO::PD2     // MISOC
};

RX65N_SPI_Driver g_RX65N_SPI_Driver;
...
void RX65N_SPI_Driver::SetSPIBits(UINT32 spi_mod, UINT32 spi_bits)
{
...
void RX65N_SPI_Driver::SetSPIClk(UINT32 spi_mod, UINT32 spi_clk)
{
    if (spi_clk == 0)
        return;
    vp_rspi prspi = g_rspi[spi_mod];
...
void RX65N_SPI_Driver::SetSPIMod(UINT32 spi_mod, const SPI_CONFIGURATION &Configuration)
{
    vp_rspi prspi = g_rspi[spi_mod];
...
void RX65N_SPI_Driver::ResetSPIMod(UINT32 spi_mod)
{
    SYSTEM.PRCR.WORD = 0xA502;
    SetMSTP(spi_mod, 1);
...
BOOL RX65N_SPI_Driver::Initialize()
{
    NATIVE_PROFILE_HAL_PROCESSOR_SPI();

    for (int i = 0; i < c_Max_SPI; i++) {
        g_RX65N_SPI_Driver.m_Enabled[i] = FALSE;
    }
    return TRUE;
}

void RX65N_SPI_Driver::Uninitialize()
{
    NATIVE_PROFILE_HAL_PROCESSOR_SPI();
}
...
// Dummy ISR()
// Not used.
void RX65N_SPI_Driver::ISR(void *Param)
{
    SPI_TRACE1(CH_BOTH, "ISR NG\r\n");
    ASSERT(0);
}

BOOL RX65N_SPI_Driver::Xaction_Start(const SPI_CONFIGURATION &Configuration)
{
    NATIVE_PROFILE_HAL_PROCESSOR_SPI();
    ...
    if (!g_RX65N_SPI_Driver.m_Enabled[spi_mod]) {
        g_RX65N_SPI_Driver.m_Enabled[spi_mod] = TRUE;
...
BOOL RX65N_SPI_Driver::Xaction_Stop(const SPI_CONFIGURATION &Configuration)
{
    NATIVE_PROFILE_HAL_PROCESSOR_SPI();
    int spi_mod = Configuration.SPI_mod;
    if (spi_mod < 0 || spi_mod >=  c_Max_SPI) {
    ...
    if (g_RX65N_SPI_Driver.m_Enabled[spi_mod]) {
        ...
        g_RX65N_SPI_Driver.m_Enabled[spi_mod] = FALSE;
    } else {
        ResetSPIMod(spi_mod);
...
BOOL RX65N_SPI_Driver::Xaction_nWrite16_nRead16(SPI_XACTION_16 &Transaction)
{
    NATIVE_PROFILE_HAL_PROCESSOR_SPI();
    UINT16 Data16;
...
BOOL RX65N_SPI_Driver::Xaction_nWrite8_nRead8(SPI_XACTION_8 &Transaction)
{
    NATIVE_PROFILE_HAL_PROCESSOR_SPI();
    UINT8 Data8;
    UINT32 spi_mod = Transaction.SPI_mod;
...
BOOL RX65N_SPI_Driver::nWrite16_nRead16(const SPI_CONFIGURATION &Configuration, UINT16 *Write16, INT32 WriteCount, UINT16 *Read16, INT32 ReadCount, INT32 ReadStartOffset)
{
    NATIVE_PROFILE_HAL_PROCESSOR_SPI();
...
BOOL RX65N_SPI_Driver::nWrite8_nRead8(const SPI_CONFIGURATION &Configuration, UINT8 *Write8, INT32 WriteCount, UINT8 *Read8, INT32 ReadCount, INT32 ReadStartOffset)
{
    NATIVE_PROFILE_HAL_PROCESSOR_SPI();
...
void RX65N_SPI_Driver::GetPins(UINT32 spi_mod, GPIO_PIN &msk, GPIO_PIN &miso, GPIO_PIN &mosi)
{
    NATIVE_PROFILE_HAL_PROCESSOR_SPI();

...
{
	ICU.IER[0x0B].BIT.IEN2 = 1; /* Enable USBR0 interrupt */
    ICU.IPR[90].BYTE    = 0x0E; /* USBR0 */
    ICU.SLIBXR137.BYTE = 62;
    /*Enable USB interrupts at high level*/
    ICU.IER[IER_PERIB_INTB137].BIT.IEN1 = 1;
    /*Set interrupt priority*/
    ICU.IPR[IPR_PERIB_INTB137].BIT.IPR = 0x0E;
...
void RX_USB_Interrupt_Uninitialize(void)
{
    ...
    ICU.IER[0x0B].BIT.IEN2 = 0; /* Disable USBR0 interrupt */
    ICU.IER[0x11].BIT.IEN1 = 0; /* Disable USBI0 interrupt */
}
...
void HandleDVST(void)
{
    switch(USB0.INTSTS0.BIT.DVSQ)
    {
        ...
        case 1:         // Default State
            USB_DEBUG1("D1\r\n");
            if((USB0.DVSTCTR0.BIT.RHST == 1)||(USB0.DVSTCTR0.BIT.RHST == 2) ) {   //KAEDE
                RX_USB_EP_Reset();
            }
            break;

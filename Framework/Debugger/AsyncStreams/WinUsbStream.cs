...
namespace Microsoft.SPOT.Debugger
{
    public class WinUsb_AsyncUsbStream : UsbStream
    {
                ...
                fixed (byte* dst = array)
                {
                    ...
                    if (remainder > 0)
                    {
                        ...
                        fixed (byte* p = byteArray)
                        {
                            ...
                            res = m_winUsbDevice.ReadPipe(Convert.ToByte(m_winUsbDevice.DeviceInfo.bulkInPipe)
                                                         , byteArray
                                                         , (uint)remainder
                                                         , ref numBytesRead
                                                         , IntPtr.Zero
        ...
        public static PortDefinition[] EnumeratePorts(bool findGenericDevices = true)
        {
            ...
            EnumeratePorts(UsbDeviceClassGuid, lst);
        ...
        {
            IntPtr devInfo = NativeMethods.SetupDiGetClassDevs(ref guid, null, 0, NativeMethods.DIGCF_DEVICEINTERFACE | NativeMethods.DIGCF_PRESENT);
